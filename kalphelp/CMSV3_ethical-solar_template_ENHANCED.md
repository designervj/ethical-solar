# CMS Website Generator — Standardized Prompt Template v3 (ENHANCED)
# Ethical Solar — Complete Reference (Example Implementation)
...
## INPUT STRUCTURE LEGEND

| Marker | Meaning | Editable? |
|--------|---------|-----------|
| `🟢 USER INPUT →` | Concrete value specific to this website | **YES — change per project** |
| `🔧 SYSTEM CONFIG →` | Architecture rule, fixed across all projects | **NO — do not modify** |
| `📝 NOTE:` | Guidance / instruction for the AI | Informational only |
...
```
1. Copy this file
2. Replace all 🟢 USER INPUT values with your website's data
3. Keep all 🔧 SYSTEM CONFIG sections as-is
4. Keep all 📝 NOTE sections as-is
5. Feed the completed prompt to the AI
```
...
# ═══════════════════════════════════════════════
# Everything marked 🟢 below is specific to THIS website.
# Change these values when generating a different website.
# ═══════════════════════════════════════════════
...
🟢 USER INPUT → Fill these fields with the client's business details.
...
🟢 USER INPUT → Set every color, font, spacing, and component token below.

📝 NOTE: These exact values are consumed by `generate-theme-css.ts` → compiled into CSS custom properties on `:root`.
...
colors:
  ...
  primaryLight: "#FF8B4A"      # Hover state (lighter orange)
  primaryDark: "#E05307"       # Active state (darker orange)
  ...
  # ── Accents ────────────────────────────────
  accent: "#2AF598"            # Neon green/teal — accents, hover borders, footer borders, icon backgrounds
...
🟢 USER INPUT → Define how each color is applied in the UI.

```
PRIMARY (#FF7020)     → Buttons (bg + text white), links, focus rings, hover borders, active states
PRIMARY LIGHT (#FF8B4A) → Button hover states
PRIMARY DARK (#E05307) → Button active/pressed states

SECONDARY (#050B2F)   → Header gradient bg (start), footer gradient bg (start), dark sections bg
ACCENT (#2AF598)      → Footer border top, decorative icon backgrounds, success indicators
ACCENT SOFT (#F3F3F3) → Light section backgrounds, card placeholder surfaces

BACKGROUND (#FFFFFF)  → Main page background
SURFACE (#F8F8F8)     → Alternate section backgrounds (gray sections)
CARD (#FFFFFF)        → Card component background

TEXT (#1F1F1F)        → Main headings, primary text
TEXT SECONDARY (#4F4640) → Body paragraphs, descriptions
TEXT MUTED (#555555)  → Placeholders, muted captions

SUCCESS (#2AF598)     → Success states, checkmarks
WARNING (#F59E0B)     → Warning badges
DANGER (#EF4444)      → Error states, required indicators

DARK BG (#050B2F)     → Footer, dark mode page bg
DARK SURFACE (#121212) → Dark mode surfaces
```
...
```yaml
typography:
  bodyFont: "'Inter', sans-serif"         # Primary UI font — loaded 100-900 weight
  headingFont: "'Epilogue', sans-serif"   # Display / heading font — loaded 100-900 weight
...
spacing:
  ...
  32: "8rem"     40: "10rem"    48: "12rem"    56: "14rem"
  64: "16rem"    72: "18rem"    96: "24rem"

radius:
  sm: "4px"        # Standard rounded-sm
  md: "6px"        # Standard rounded-md
  lg: "12px"       # Large rounded-lg
  xl: "16px"       # Card rounded-xl
  "2xl": "24px"    # Section / large container
...
shadow:
  ...
  xl: "0 15px 25px rgba(0,0,0,.2)"
  "2xl": "0 25px 50px rgba(0,0,0,.25)"
  custom: "0 10px 40px rgba(0,92,151,0.05)"
```

### A2e. Layout & Component Tokens

```yaml
layout:
  container: "1024px"            # max-w-5xl (1024px) for standard container
  containerLg: "1152px"          # max-w-6xl (1152px) for container-lg
  containerXl: "1280px"          # max-w-7xl (1280px) for wide sections
  navbarHeight: "80px"
  sectionPadding: "5rem"         # py-20
  sectionPaddingMd: "5rem"       # md:py-20
  gapDefault: "1.5rem"           # gap-6
  gapLg: "2.5rem"                # gap-10
...
# ── Gradients (for reference in component styling) ──
gradients:
  primaryBrand: "linear-gradient(90deg, #23469B 0%, #26A187 100%)"
  darkSection: "linear-gradient(90deg, #050B2F 0%, #121212 100%)"
  tealAccent: "linear-gradient(90deg, #00b09b, #0088cc)"
  overlay: "linear-gradient(to bottom, rgba(2,24,20,0.15), transparent)"
```
...
🟢 USER INPUT → Define every page, its route, data source, and metadata.

### A3a. Navigation Structure
...
| # | Page Name | Slug | Type | Source File | metaTitle | metaDescription |
|---|-----------|------|------|-------------|-----------|-----------------|
| 1 | Home | `/` | Static JSON | `homePage.json` | Ethical Solar — Solar Built to Last | Trusted solar panel installation across South East Queensland. |
| 2 | Residential | `/residential` | Static JSON | `residentialPage.json` | Residential Solar — Ethical Solar | Custom rooftop solar for Australian homes. |
| 3 | Commercial | `/commercial` | Static JSON | `commercialPage.json` | Commercial Solar — Ethical Solar | High-performance commercial solar systems. |
| 4 | About Us | `/about-us` | Static JSON | `aboutUsPage.json` | About Us — Ethical Solar | Pioneering Australia with integrity and purpose. |
| 5 | Blog | `/blog` | Static JSON | `blogPage.json` | Solar News & Insights — Ethical Solar | Renewable energy, battery storage, and solar updates. |
| 6 | Blog Post | `/blog/[slug]` | Dynamic (MongoDB) | — | — | — |
| 7 | Contact Us | `/contact-us` | Static JSON | `contactPage.json` | Contact Us — Ethical Solar | Get in touch for a free solar assessment. |
...
🟢 USER INPUT → Define every section/block for each page. The block types, layouts, and all content are specified here.

📝 NOTE: Each block follows the `PageBlock` interface: `{ id, type, layout, adminTitle, props, content? }`.
📝 NOTE: All text values are plain strings (no HTML). Rich text is rendered via `EditableText` component.

### A4a. Available Block Types Reference

| Block Type | Layout Options | Purpose |
|------------|---------------|---------|
...
homePage:
  ...
  - id: services-001
    type: service-cards
    layout: grid
    adminTitle: What We Do Overview
    props:
  ...
  - id: what-we-do-002
    ...
    layout: grid
    adminTitle: What Ethical Solar Does (Detailed)
    props:
...
### A4c. Residential Page — Section Blocks
    ...
    props:
      ...
      items:
        ...
        - title: "Government Incentives Available"
          description: "Eligible homeowners can benefit from the Small-scale Renewable Energy Scheme (SRES) which provides STCs that reduce upfront system costs. You may also be eligible for around a 30% discount on the upfront cost of installing a battery system through the Cheaper Home Batteries Program."
          type: "highlight"
...
### A4d. Commercial Page — Section Blocks
...
commercialPage:
    ...
    props:
      ...
      services:
        - title: "Rooftop Commercial Solar"
          description: "Flat or pitched rooftop systems designed for offices, retail centres, schools, and industrial buildings."
          bullets:
        ...
        - title: "Carport Solar Structures"
          description: "Solar carports combine covered parking with energy generation — turning underused car parks into productive assets."
          bullets:
        ...
        - title: "Ground Mount Systems"
          description: "For properties with available land, ground-mounted systems offer flexibility in orientation and scale."
          bullets:
        ...
        - title: "Warehouse & Industrial Solar"
          description: "Large roof areas on warehouses and industrial facilities are ideal for high-capacity solar."
          bullets:
    ...
    props:
      ...
      steps:
        - step: "01"
          ...
          description: "We review your electricity bills, operational hours, and energy goals."
        - step: "02"
          title: "System Design & Proposal"
          description: "Custom system design with full financial modelling, savings, payback period, and ROI."
        - step: "03"
          title: "Approvals & Permitting"
          description: "We manage all council approvals, network applications, and grid connection paperwork."
        - step: "04"
          title: "Professional Installation"
          description: "CEC-accredited teams complete the project with minimal disruption to your business."
        - step: "05"
          title: "Commissioning & Monitoring Setup"
          description: "System commissioned, tested, connected to our monitoring platform."
        - step: "06"
          title: "Ongoing Support & Maintenance"
          description: "Scheduled maintenance checks, performance reviews, and reactive support."
  ...
  - id: com-battery-001
    ...
    props:
      ...
      description: "A commercial solar system paired with battery storage delivers maximum financial benefit. Store solar energy generated during the day and deploy it during peak tariff periods."
      bullets:
        - "Reduce peak demand charges significantly"
  ...
  - id: com-panels-001
    ...
    props:
      ...
      subheading: "We exclusively source Tier-1 panels from manufacturers with proven 25-year track records."
      panels:
        - brand: "Q CELLS"
          detail: "Q.PEAK DUO BLK ML-G10+ series — high efficiency monocrystalline panels with anti-LID and anti-LeTID technology."
        - brand: "REC Group"
          detail: "REC Alpha series — twin peak solar cell design delivering up to 405 W output. Excellent low-light performance."
        - brand: "Jinko Solar"
          detail: "Tiger Neo series — n-type TOPCon technology with class-leading efficiency above 22%."
  ...
  - id: com-finance-001
    ...
    props:
      ...
      description: "Commercial solar is one of the highest-returning capital investments available to Australian businesses today."
      stats:
        - value: "4–7 years"
      ...
      footnote: "Government STCs and LGCs reduce upfront costs further. Commercial finance options available."
    ...
    props:
      ...
      items:
        - question: "How much can a commercial solar system save my business?"
          answer: "Most commercial customers see a 30–60% reduction in electricity bills, with full payback typically achieved within 4–7 years."
        - question: "Do we need to shut operations down during installation?"
          answer: "In most cases, no. Our installation teams work around active business operations."
        - question: "What size system does my business need?"
          answer: "Determined by your energy consumption, roof/ground space, and budget. We conduct a detailed energy audit."
        - question: "Are there government incentives available for commercial solar?"
          answer: "Yes. SRES and LGCs are available depending on system size."
        - question: "What warranties come with a commercial solar install?"
          answer: "Up to 25-year product and performance warranties on panels, 10-year inverter warranties, and 10-year workmanship warranty."
...
### A4e. About Us Page — Section Blocks
    ...
    props:
      ...
      items:
        ...
        - title: "Certified & Experienced Installers"
          description: "Every installation is completed by Clean Energy Council accredited professionals."
        - title: "Transparent Pricing"
          description: "No hidden fees, no surprise add-ons. What we quote is what you pay."
        ...
        - title: "Sustainable Practices"
          description: "We offset our operational footprint and prioritise environmentally responsible installation."
        - title: "Customer-First Approach"
          description: "We only recommend a solar system if it genuinely makes financial and practical sense."
...
### A4g. Contact Page — Section Blocks
...
🟢 USER INPUT → Define global components shared across all pages (header, footer).

### A5a. Header Configuration
...
header:
  ...
  style:
    ...
    textColor: "#FFFFFF"
    hoverTextColor: "#FF7020"
```

### A5b. Footer Configuration
...
🟢 USER INPUT → Define database collections for dynamic content.
...
🟢 USER INPUT → Every form on the website is defined entirely in JSON via `section.props.form`. All forms follow the same structure below.

📝 NOTE: Forms are rendered dynamically — no field names are hardcoded. See Section B for the DynamicForm rendering spec.

### A7a. Form JSON Structure
...
📝 NOTE: Unknown field types must be ignored gracefully — never break rendering.
...
# ═══════════════════════════════════════════════
# 🔧 Everything below is FIXED architecture.
# Do NOT modify. Applies identically to ALL CMS websites.
# ═══════════════════════════════════════════════
...
🔧 SYSTEM CONFIG → Act as an Elite Full-Stack Next.js 15 Architect, CMS Specialist, and Design System Expert. Generate a complete, production-ready corporate/portfolio/marketing/blog CMS website.
...
🔧 SYSTEM CONFIG → Generate ALL of the following files:

### B4a. `src/styles/globals.css`
...
### B4b. `src/lib/theme/theme.config.ts`
...
### B4c. `scripts/generate-theme-css.ts`

Reads `theme.config.ts`, replaces `{path.placeholders}` in a CSS template, writes `globals.css`.

### B4d. Page JSON files (from SECTION A4)

Generate one JSON file per static page in `src/lib/data/pages/` following the PageBlock schema:
...
### B4e. Header + Footer JSON

Generate `headerData.json` and `footerData.json` from SECTION A5.

### B4f. MongoDB types (`src/lib/db/types.ts`)

Generate TypeScript interfaces for all dynamic collections listed in SECTION A6.

### B4g. Redux Store

Pages slice with `setCurrentPageBySlug`, `updatePageField`, `setEditableMode`. Store with Redux Toolkit.

### B4h. Shared UI Components
...
### B4i. Page Section Components

For each unique `type` used in SECTION A4, generate a render component:

| Component | Layouts | Used On |
|-----------|---------|---------|
| `HeroSection` | split-overlay | Home |
| `HeroForm` | split | Residential |
| `HeroPageHeader` | centered | Commercial, About, Blog, Contact |
| `LogoCloud` | slider | Home |
| `ServiceCards` | grid | Home |
| `ServiceCardsBullets` | grid | Commercial |
| `LetsTalkStrip` | centered | Home |
| `WhatWeDoGrid` | grid | Home |
| `AboutVideo` | split | Home |
| `WorkGallery` | grid | Home |
| `CtaCentered` | centered | Home, Residential, Commercial, About |
| `BenefitsGrid` | grid | Residential |
| `ProcessSteps` | grid | Residential |
| `TextColumns` | two-column | Residential |
| `VerticalTimeline` | left-border | Residential |
| `FaqAccordion` | centered | Residential, Commercial |
| `ProcessCards` | list | Commercial |
| `SplitHighlight` | split | Commercial, About |
| `ProductCards` | grid | Commercial |
| `CardHighlight` | full-width | Commercial |
| `TextImageSplit` | split | About |
| `CardGrid` | grid | About |
| `TextCentered` | centered | About |
| `ChecklistGrid` | grid | About |
| `BlogCards` | grid | Blog |
| `ContactSplit` | split | Contact |
| `DynamicForm` | config-driven | Renders forms entirely from JSON |

### B4j. API Proxy Route — `src/app/api/[[slug]]/route.ts`

Single catch-all route that:
- Proxies all HTTP methods to `KALZERO_PUBLIC_API_URL` + path
- Intercepts `PUT/PATCH /api/media/[id]` for direct MongoDB updates
- Forwards form submissions to FastAPI backend

### B4k. `src/middleware.ts` (multi-language only)

Locale-aware middleware using URL prefix pattern.

### B4l. `src/lib/i18n/locale.ts` (multi-language only)

Export `getLocalizedString` helper.

### B4m. Dynamic Form Section Component

`FormSection` renders forms entirely from JSON config:
- Reads `section.props.form` for field definitions and settings
- Reads `section.props.formHeading`, `formDescription`, `successHeading`, `successDescription`, `successButtonText`
- Uses `section.layout` for field grid (one_column/two_column/three_column)
- Renders all 17 field types from the dynamic field definitions
- Client-side validation: required, email, phone, pattern, min/max
- Inline validation messages below each field
- POST to `/api/form-data` with body `{ pageSlug, pageTitle, sectionId, formId, formName, submission: { fieldName: value } }`
- Auto `multipart/form-data` when file fields present
- Submit disabled + loading indicator during submission
- On success: hide form, show success state
- On error: preserve values, show errors
- Full accessibility: id, name, label, aria-*, keyboard nav
- Unknown field types ignored gracefully
...
| DynamicForm | `src/components/sections/DynamicForm.tsx` | Renders forms from JSON config |
...
> ✅ **Ready to generate.** Feed this complete prompt to the AI.
> To reuse for another website: replace all 🟢 USER INPUT values with the new client's data, keep 🔧 SYSTEM CONFIG sections untouched.
