          ...
          {benefits.slice(0, 2).map((benefit: any, i: number) => {
            ...
            return (
              <div key={i} className="group bg-gray-50 hover:bg-primary/10 border border-gray-100 hover:border-primary/20 rounded-2xl p-8 transition-all duration-300">
                <div className="w-12 h-12 bg-primary/20 group-hover:bg-emerald-200 rounded-xl flex items-center justify-center mb-5 transition-colors duration-300">
          ...
          {steps.map((step: any, i: number) => {
            ...
            return (
              <div key={i} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 items-start">
                <div className={`w-14 h-14 bg-primary/20 text-primary rounded-full flex items-center justify-center flex-shrink-0`}>
                  <Icon size={28} />
                </div>
                <div>
...
const ProcessStep = ({ number, title, desc }: { number: string; title: string; desc: string }) => (
  <div className="flex gap-6 relative group">
    <div className="flex-shrink-0 w-12 h-12 rounded-full bg-primary/20 text-primary flex items-center justify-center font-serif font-bold text-xl z-10 relative group-hover:bg-primary-hover group-hover:text-white transition-colors duration-300">
      {number}
    </div>
